-- validate-rls-operativo.sql — POL-33 RLS tablas operativas (020–029)
--
-- Prerrequisitos: migraciones 001–030, validate-phase1.sql, validate-rls-multitenant.sql
--
-- Simulación de sesión:
--   SET LOCAL ROLE authenticated;
--   SELECT set_config('request.jwt.claim.sub', '<id_auth>', true);
--
-- Ejecutar conectado como postgres (o rol con GRANT authenticated + bypass RLS para seed).
...
-- Stock (como postgres; RLS no aplica al owner en INSERT directo — seed fuera de rol authenticated)
...
-- ---------------------------------------------------------------------------
-- Prueba 7: Operador ACME-01 NO ve productos de otra cuenta (BETA)
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM producto WHERE codigo_cuenta = 'BETA-01'
    ) INTO v_ve_beta;
...
-- ---------------------------------------------------------------------------
-- Prueba 8: Operador authenticated NO puede INSERT en warehouse_state
-- id_auth: 33333333-3333-3333-3333-333333333333
-- ---------------------------------------------------------------------------
DO $$
BEGIN
...
-- ---------------------------------------------------------------------------
-- Prueba 9: Configurador ve catálogos de múltiples cuentas
...
BEGIN
    ...
    PERFORM test_rls.assert_true(
        '9 configurador ve catálogos ACME y BETA',
        v_cuentas_distintas >= 2
    );
END;
$$;

-- ---------------------------------------------------------------------------
-- Prueba 10: Custodio ve warehouse_state solo de su bodega asignada (CENTRAL)
...
-- ---------------------------------------------------------------------------
-- Prueba 11: Helper auth_wms_puede_ver_fila_operativa (030)
-- ---------------------------------------------------------------------------
DO $$
DECLARE
    v_ok boolean;
BEGIN
    ...
    SELECT auth_wms_puede_ver_fila_operativa(
        'ACME-01',
...
SELECT 'validate-rls-operativo.sql completado' AS resultado;
